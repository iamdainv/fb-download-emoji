...
const Constants = require('./constants/size');
const emojiData = require('./utils/emoji_data.js');
...
function getUrl(a, c, d) {
	...
	a = h(f, 317426846);
	return (
		'https://static.xx.fbcdn.net/images/emoji.php/v9' +
		'/' +
		d +
		a +
		'/' +
...
if (!size || size.length === 0) {
	console.error(
		chalk.red(
			`You must specify emoji size to download. Use one of these value: ${_supportSizesStr}`
		)
	);
	return;
	...
	console.error(
		chalk.red(
			`The size: ${_sizeValue} is not supported. Use one of these value: ${_supportSizesStr}`
		)
	);
	return;
}
...
if (!toDir || toDir.length === 0) {
	console.error(
		chalk.red(`You must specify a target folder to save emoji.`)
	);
	return;
}
...
console.log(
	chalk.yellow(
		`Started downloading Facebook emoji size: ${size}. Please wait...`
	)
);
...
//run node index.js size folderSave

emojiData.map(async function(cat) {
	cat.emoji.map(async function(emoji) {
		_emojiCount++;
		const name = unicodeUtils.getKeyFromCodepoints(
			emoji.codepoint
		);
		...
		if (_done === _emojiCount) {
			console.log(
				chalk.green(
					`Success downloaded ${_done} Emojis of size ${size}.`
				)
			);
		}
	});
